...
description: One command, twelve checks, run against the live Nimiq chain. Real payouts, real refusals, saved output.
---
...
Nimiq chain, and the output of each run is saved in the repository.
...
The local run is a testnet tool by design. It moves real test NIM out of a real test wallet,
so running it never spends the mainnet pool.
...
pay, and reads that payment back off the chain the deployment says it is on. Against Vettai's
own deployment that chain is Nimiq mainnet, and the run pays one real 0.5 NIM quest. The
checks that need the host's own database or its configured caps are skipped rather than
faked, and the summary counts them as skipped.

Mainnet proof: 0.5 NIM, memo `vettai:e47d1142`, block 61817025, [`3dca159e...d18113`](https://nimiq.watch/#3dca159edf4df9ec23a42aafb5e5fa95de6dda82be7405c09064a0d039d18113)
...
This is a testnet run, kept as the record of all twelve checks, because five of them need the
host's own database and its configured caps. It is the file as it was written, not a summary
of it.
