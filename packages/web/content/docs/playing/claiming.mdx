...
Every one of those is exercised against the live chain by the
[prove-it run](/docs/trust/proof).

## Getting help with a payout

If a claim comes back as failed, or a shop order ran out of time, post in the [Nimiq Mini Apps
community on Skool](https://www.skool.com/miniappscompetition) with the memo shown on that row. The memo carries the quest or order id,
the treasury keeps the record either way, and it is settled by hand.
